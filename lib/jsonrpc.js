...
module.exports = class Jsonrpc {
	...
	static isNotification (request) { return isNotification(request); }
	static responseIsValid (response) { return responseIsValid(response); }
	...
	constructor (options = {}) {
		...
		Object.defineProperty(this, 'params', {
			enumerable: false,
			get: function () {
				if (isObject(request)) return request.params;
			}
		});
		Object.defineProperty(this, 'method', {
			enumerable: false,
			get: function () {
				if (isObject(request)) return request.method;
			}
		});
...
function methodIsValid (val) {
	return val && ('string' === typeof val && Boolean(val.trim())) && !val.startsWith('rpc.');
}
